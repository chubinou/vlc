...
static void on_player_vout_list_changed(vlc_player_t *player, enum vlc_player_list_action action, vout_thread_t *vout, void *data)
{
    ...
    size_t i_vout = 0;
    vout_thread_t **vouts = vlc_player_GetVouts(player, &i_vout);
...
InputManager::VoutPtrList InputManager::getVouts() const
{
    ...
    size_t i_vout;
    {
        ...
        if( !vlc_player_IsStarted( d->m_player ) )
            return VoutPtrList{};
        i_vout = 0;
        pp_vout = vlc_player_GetVouts( d->m_player, &i_vout );
        if ( i_vout <= 0 )
            return VoutPtrList{};
    }
...
InputManager::VoutPtr InputManager::getVout()
{
    ...
    vlc_player_locker lock{ d->m_player };
    size_t count = 0;
    vout_thread_t** vouts = vlc_player_GetVouts( d->m_player, &count );
...
InputManager::AoutPtr InputManager::getAout()
{
    Q_D(InputManager);
    vlc_player_locker lock{ d->m_player };
    return AoutPtr( vlc_player_GetAout( d->m_player ), false );
}
